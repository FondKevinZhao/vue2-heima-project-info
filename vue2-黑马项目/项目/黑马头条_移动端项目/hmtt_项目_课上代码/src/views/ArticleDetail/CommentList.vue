<template>
  <div>
    ...
    -> 触发load事件执行 -> loading: 为true -> 底部加载中文字出现 -> 内部认为现在处于加载中状态
     ...
     原因: list中列表内容是异步网络请求回来的, 标签挂载时, 没有高度, list就会判定触底执行一次
     onLoad方法
...
<script>
// 听课思想:
// 疑问: 老师我背下来套路和流程和代码, 还是说我真的自己是会了?
// 遇到类似的需求, 看看自己有没有经验
// 区分:
// 反例(背下来): 老师绑定了点击事件, 给个方法, 传true和false下去, 脑子里的活动是这样的 你就是在背代码(熟悉)
// 正例(理解-过程): 熟悉+理解+过程 (验证: 给别人讲明白)
// 扩展思路2个方式: 学的相当不错~~~
import {
  commentsListAPI,
  commentDisLikingAPI,
...
// 分享到微博 -> 微博开发平台 -> 注册账号, 调用人家平台封装的js代码, 调用他们的接口来实现分享功能
</script>
